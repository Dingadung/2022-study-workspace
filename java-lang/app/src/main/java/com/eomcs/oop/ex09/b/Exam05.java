...
public class Exam05 {
  public static void main(String[] args) {
    ...
    // 수퍼클래스에 정의된 static메서드는,
    // 다음과 같이 서브 클래스를 통해 호출할 수 있다.
    MyInterface5Impl.m2();

    // 또는 다음과 같이 인스턴스 레퍼런스를 통해 호출할 수 있다.
    // 물론, 스태틱메서드는 가능한 그 메서가 선언된 클래스 이름으로호출하는  것이 좋다.
    obj.m2();

    // 그러나, 인터페이스에 정의된 static메서드는 인터페이스를 구현한클래스를 통해 호출할 수 없다!
    ...
    // 인터페이스 자체에서 static 메소드인터페이스 이름으로 바로 부를 수 있지만, 이 인터페이스를 구현한 클래스명을 통해서는 부를 수 없다.
  }
